...
#define CONDITION_NUMBER 10000.00
...
//printf("WORLD RANK/SIZE: %d/%d \t COMM1 RANK/SIZE: %d/%d\n", rank, size, rank_1, size_1);
        ...
        if (!(rank_1)){
        	...
        	sprintf(filename1, "%d_%d/Condition_Number_%.2f/OUT_Solve_QCQP_Our_Algorithm_Parallel_%d_%d_Core_%d_x", M, N, CONDITION_NUMBER, M, N, T1+T2);
            if(!(fp1 = fopen(filename1,"r"))){
                printf("Cannot open this file\n");
                exit(0);
            }
            for (n_col = 0; n_col < N; n_col++){
                fscanf(fp1, "%lf", &solution[n_col]);
            }
            fclose(fp1);
        }
        MPI_Scatter(solution, n_t, MPI_DOUBLE, x, n_t, MPI_DOUBLE, 0, comm_1);
...
//printf("WORLD RANK/SIZE: %d/%d \t COMM2 RANK/SIZE: %d/%d\n", rank, size, rank_2, size_2);
        ...
        if (!(rank_2)){
        	sol_lam = (double *)calloc(M, sizeof(double));
        	sprintf(filename1, "%d_%d/Condition_Number_%.2f/OUT_Solve_QCQP_Our_Algorithm_Parallel_%d_%d_Core_%d_lam", M, N, CONDITION_NUMBER, M, N, T1+T2);
        	if(!(fp1 = fopen(filename1,"r"))){
                printf("Cannot open this file\n");
                exit(0);
            }
            for (m = 0; m < M; m++){
                fscanf(fp1, "%lf", &sol_lam[m]);
            }
            fclose(fp1);
        }
        MPI_Scatter(sol_lam, m_t, MPI_DOUBLE, lam, m_t, MPI_DOUBLE, 0, comm_2);
...
//printf("%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\n", rho[1], rho[2], rho[3], rho[4], rho[5]);
    ...
    if (rank < T1){
    	...
    	if (!(rank_1)){
    		...
    		fclose(fp1);
    		//Output x:
    		sprintf(filename1, "%d_%d/Condition_Number_%.2f/OUT_Solve_QCQP_Our_Algorithm_Parallel_%d_%d_Core_%d_x", M, N, CONDITION_NUMBER, M, N, T1+T2); //OUT_Solve_QCQP_Our_Algorithm_Parallel_M_N_T1+T2_x is the file containing x
    		if(!(fp1 = fopen(filename1,"w"))){
    			printf("Cannot open this file\n");
    			exit(0);
    		}
    	...
    	MPI_Gather(lam, m_t, MPI_DOUBLE, sol_lam, m_t, MPI_DOUBLE, 0, comm_2);

    	if (!(rank_2)){
    		//Output lam:
    		sprintf(filename1, "%d_%d/Condition_Number_%.2f/OUT_Solve_QCQP_Our_Algorithm_Parallel_%d_%d_Core_%d_lam", M, N, CONDITION_NUMBER, M, N, T1+T2); //OUT_Solve_QCQP_Our_Algorithm_Parallel_M_N_T1+T2_lam is the file containing lam
    		if(!(fp1 = fopen(filename1,"w"))){
    			printf("Cannot open this file\n");
    			exit(0);
    		}
    		for (n_row = 0; n_row < M; n_row++){
    			fprintf(fp1, "%.16lf\n", sol_lam[n_row]);
    		}
    		fclose(fp1);
    	}
