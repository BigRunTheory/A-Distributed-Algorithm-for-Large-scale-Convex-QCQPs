...
printf("Calculation of Matrix Norm is OK!\n");
    ...
    sprintf(filename1, "%d_%d/OUT_Solve_QCQP_Our_Algorithm_Tolerance_%d_%d", M, N, M, N); //OUT_Solve_QCQP_Our_Algorithm_Tolerance_M_N is the file containing the tolerance of each iteration
    if(!(fp1 = fopen(filename1,"w"))){
    	printf("Cannot open this file\n");
    	exit(0);
    }
    sprintf(filename2, "%d_%d/OUT_Solve_QCQP_Our_Algorithm_Epsilon_%d_%d", M, N, M, N); //OUT_Solve_QCQP_Our_Algorithm_Weight_M_N is the file containing the epsilons of each iteration
    if(!(fp2 = fopen(filename2,"w"))){
        printf("Cannot open this file\n");
        exit(0);
    }
    sprintf(filename3, "%d_%d/OUT_Solve_QCQP_Our_Algorithm_Rho_%d_%d", M, N, M, N); //OUT_Solve_QCQP_Our_Algorithm_Rho_M_N is the file containing the rhos of each iteration
...
//printf("%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\n", rho[1], rho[2], rho[3], rho[4], rho[5]);
    ...
    //Output Solution:
    sprintf(filename1, "%d_%d/OUT_Solve_QCQP_Our_Algorithm_%d_%d", M, N, M, N); //OUT_Solve_QCQP_Our_Algorithm_M_N is the file containing the solution
    if(!(fp1 = fopen(filename1,"w"))){
    	printf("Cannot open this file\n");
    	exit(0);
    }
