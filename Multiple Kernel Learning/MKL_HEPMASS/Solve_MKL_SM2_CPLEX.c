...
int Solve_CPLEX (double *x, double* lam, double *objval, int *solstat, double *obj, double *qsepvec, int ccnt, int rcnt, int nzcnt, double *rhsval1, char *sense1, int *rmatbeg, int *rmatind, double *rmatval, int *linnzcnt, int *quadnzcnt, double *rhsval2, char sense2, int **linind, double **linval, int **quadrow, int **quadcol, double **quadval){
    int status = 0;
    CPXENVptr env = NULL;
    CPXLPptr lp  = NULL;
    char errmsg[1024];
...
int main (int argc, char *argv[]){
    ...
    //Quadratic Objective
    double *qsepvec = (double *)calloc(N, sizeof(double));
...
printf("Initiation of Constraints of QCQP is OK!\n");       
    ...
    start = time(NULL);
    Solve_CPLEX (x, lam, &objval, &solstat, obj, qsepvec, ccnt, rcnt, nzcnt, rhsval1, sense1, rmatbeg, rmatind, rmatval, linnzcnt, quadnzcnt, rhsval2, sense2, linind, linval, quadrow, quadcol, quadval);
    end = time(NULL);
    ...
    sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM2_CPLEX", FOLDER, M);
    ...
    sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM2_CPLEX_Alpha", FOLDER, M);
    if(!(fp1 = fopen(filename1,"w"))){
        printf("Cannot open this file\n");
        exit(0);
    }
    ...
    sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM2_CPLEX_Lambda", FOLDER, M);
    if(!(fp1 = fopen(filename1, "w"))){
        printf("Cannot open this file\n");
        exit(0);
    }
