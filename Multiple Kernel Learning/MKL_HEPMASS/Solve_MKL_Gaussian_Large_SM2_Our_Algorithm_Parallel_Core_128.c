...
#define T2 9//number of threads for dual variables
...
printf("Calculation of Matrix Norms is OK!\n");
        ...
        if (!(rank_1)){
            ...
            //Open File
            sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM2_Our_Algorithm_Parallel_Core_%d_Tolerance", FOLDER, M, T1); //OUT_Solve_MKL_HM_Our_Algorithm_Parallel_Core_T1_Tolerance is the file containing the tolerance of each iteration
    ...
    for (k = 0; k < MAX_ITER; k++){
        ...
        if (rank < T1){
        	//Initiate Gradient_x with x/C + q_obj:
        	for (n_col = 0; n_col < n_t; n_col++){
        		grad_x[n_col] = (1.0/C)*x[n_col] + q_obj[n_col];
        	}
        	//Calculate A*x:
...
//printf("%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\n", rho[1], rho[2], rho[3], rho[4], rho[5]);
        ...
        //Update Gradient and Residual:
        if (rank < T1){
        	//Initiate Gradient_x with y/C + q_obj:
        	for (n_col = 0; n_col < n_t; n_col++){
        		grad_x[n_col] = (1.0/C)*y[n_col] + q_obj[n_col];
        	}
        	//Calculate A*y:
        	vector_primal_part = 0.0;
        	for (n_col = 0; n_col < n_t; n_col++){
    ...
    if (rank < T1){
    	objval_part = 0.0;
    	for (n_col = 0; n_col < n_t; n_col++){
    		objval_part += (1/(2*C))*pow(x[n_col], (double)2) + q_obj[n_col]*x[n_col];
    	}
    ...
    if (!(rank)){
    	...
    	//Output Solution:
    	sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM2_Our_Algorithm_Parallel_Core_%d", FOLDER, M, T1);
    	if(!(fp1 = fopen(filename1,"w"))){
    		printf("Cannot open this file\n");
    		exit(0);
    	}
        ...
        sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM2_Our_Algorithm_Parallel_Core_%d_Alpha", FOLDER, M, T1);
        if(!(fp1 = fopen(filename1,"w"))){
            printf("Cannot open this file\n");
            exit(0);
        }
        ...
        sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM2_Our_Algorithm_Parallel_Core_%d_Lambda", FOLDER, M, T1);
        if(!(fp1 = fopen(filename1,"w"))){
            printf("Cannot open this file\n");
            exit(0);
        }
