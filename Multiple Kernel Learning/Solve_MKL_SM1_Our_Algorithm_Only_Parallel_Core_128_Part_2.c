...
#define TOLERANCE_1 0.004
#define TOLERANCE_2 0.0004
#define LARGE 100.0
#define C 2.0
    ...
    if (rank < T1){
        ...
        if (!(rank_1)){
            ...
            for (n_col = 0; n_col < N; n_col++){
                fscanf(fp1, "%lf", &solution_x[n_col]);
            }
            fclose(fp1);
        ...
        MPI_Scatter(solution_x, n_t, MPI_DOUBLE, x, n_t, MPI_DOUBLE, 0, comm_1);
...
printf("Calculation of Matrix Norms is OK!\n");
    ...
    if (rank >= T1){
        ...
        MPI_Scatter(solution_lam, m_t, MPI_DOUBLE, lam, m_t, MPI_DOUBLE, 0, comm_2);
