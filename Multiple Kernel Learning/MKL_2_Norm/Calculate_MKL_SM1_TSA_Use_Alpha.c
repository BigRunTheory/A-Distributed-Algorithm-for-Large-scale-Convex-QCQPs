...
int main (int argc, char *argv[]){
    ...
    sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM1_%s_Lambda", FOLDER, M, METHOD);
    ...
    sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM1_%s_Alpha", FOLDER, M, METHOD);
    if(!(fp1 = fopen(filename1,"r"))){
        printf("Cannot open this file\n");
        exit(0);
    }
...
printf("The offset is: %.16lf.\n", offset);  
    ...
    sprintf(filename1, "%s/%d/OUT_Solve_MKL_SM1_%s_TSA", FOLDER, M, METHOD);
